  ...
  bot.onText(/^\/milestones(@\w+)?$/i, (msg) => {
    ...
    try {
      ...
      if (fiTarget <= 0 || fiMonths == null) {
        out += `Financial Indep:  unavailable\n`;
      } else {
        out += `Financial Indep:  ${futureMonthLabel(fiMonths)}\n`;
      }
      ...
      for (const t of targets) {
        ...
        const label = `Net Worth ${money(t)}:`.padEnd(18);
